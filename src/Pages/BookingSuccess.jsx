...
const BookingSuccess = () => {
  ...
  const { serviceName, estimatedCost, serviceTime, bookingId, status } = state;
      ...
      <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl max-w-md w-full text-center p-8 relative overflow-hidden">
        ...
        <p className="text-gray-300 mb-6 text-sm">
          {currentUser?.role === "admin"
            ? "This request is now available inside the admin dashboard."
            : "Thanks for trusting RepairMate. Your request has been submitted successfully."}
        </p>
        ...
        <div className="bg-black/30 rounded-xl p-5 text-left text-sm space-y-3">
          ...
          <div>
            ...
            <span className="text-white font-semibold">
              {typeof estimatedCost === "number" ? `₹${estimatedCost}` : estimatedCost}
            </span>
          </div>
          <div>
          ...
          <div>
            ...
            <span className="text-white font-semibold">{status || "Pending"}</span>
          </div>
        </div>
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
          <button
            onClick={() => navigate("/")}
            className="bg-green-500 hover:bg-green-600 text-black font-bold py-2 px-6 rounded-full transition duration-300"
          >
            Back to Home
          </button>
          {currentUser?.role === "admin" && (
            <button
              ...
              className="border border-white/20 bg-white/5 hover:bg-white/10 text-white font-bold py-2 px-6 rounded-full transition duration-300"
            >
              Open Admin Panel
            </button>
          )}
